...
export default function ProjectCard({ project }) {
  ...
  return (
    <div className="glass rounded-2xl overflow-hidden card-hover group h-full flex flex-col relative before:absolute before:inset-0 before:rounded-2xl before:border before:border-transparent before:transition-all hover:before:border-neon-cyan/50 hover:before:shadow-[0_0_20px_rgba(0,243,255,0.2)] isolate">
      ...
      {project.image ? (
        ...
        <div className={`relative w-full h-36 bg-gradient-to-br ${pickGradient(project.title)} flex items-center justify-center overflow-hidden`}>
          <svg className="w-10 h-10 text-white/30 transform group-hover:scale-110 transition-transform duration-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="p-5 flex flex-col flex-1 z-10">
        {/* ── Titre ── */}
        <h3 className="text-base font-bold mb-2 text-white group-hover:text-neon-cyan transition-colors duration-200 line-clamp-1">
          {project.title}
        </h3>
        ...
        {/* ── Liens GitHub / Démo ── */}
        <div className="flex items-center justify-between pt-3 mt-auto border-t border-white/[0.08]">
          <div className="flex items-center gap-4">
            {project.github && (
              <span className="flex items-center gap-1.5 text-[12px] font-medium text-gray-400 group-hover:text-white transition-colors">
            ...
            {project.demo && (
              <span className="flex items-center gap-1.5 text-[12px] font-bold text-neon-cyan group-hover:text-cyan-300 transition-colors drop-shadow-[0_0_5px_rgba(0,243,255,0.4)]">
          ...
          <span className="text-[12px] font-semibold text-gray-500 opacity-0 transform translate-x-3 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-300">
            Explorer →
          </span>
        </div>
      </div>
